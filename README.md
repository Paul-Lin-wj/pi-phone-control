...
约束机制：模型接触不到裸 adb/bash 命令；`adbShell` 整条命令单引号字面传递（宿主机零展开）；inject 走 `su -c`（固定命令模板）；`tool_call` 拦截 bash 中的 adb/su/sudo；**bash 混淆确认门**——含 eval/变量拼接/base64 解码/管道进 shell 特征的命令弹 TUI 确认框由用户放行（print/json 无 UI 模式默认拒绝）；横幅任务期间强制常显（用户暂停/终止的唯一入口，仅截图/触控瞬间避让）；`session_shutdown` 自动收尾（输入法恢复 + 撤横幅）。
...
- bash 硬拦是命令文本正则（`\badb\b`/`\bsu\b`/`\bsudo\b`），混淆形态已由确认门兜住（eval/变量拼接/base64 解码/管道进 shell）；剩余边界：解释器内联代码（如 `python -c` 内拼子进程调 adb）不含这些特征，仍靠模型自身拒绝（端到端实测拒绝）
