...
- `~/.pi/agent/settings.json`：defaultModel（当前 glm-5.3-flash）+ `defaultThinkingLevel: "medium"`（2026-08-28 速度调优：原 max 每步深推理拖慢节奏，off 又太直给——屏幕有意外内容时会照字面跑偏；medium 平衡思考与速度，复杂任务临时 `pi --thinking max`）
